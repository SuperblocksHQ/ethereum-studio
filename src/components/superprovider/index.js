...
export default class SuperProvider {
    ...
    _initIframe=()=>{
        ...
        if(this.iframe.contentWindow) {
            this.iframe.contentWindow.postMessage({type:"init",channel:this.that.id}, '*');
        }
    ...
    _onMessage=(event)=>{
        ...
        const send=(payload, endpoint, callback)=>{
            ...
            if(endpoint.toLowerCase()=="http://superblocks-browser") {
                this.that.props.functions.EVM.getProvider().sendAsync(payload, callback);
            }
            else {
                fetch(endpoint, {
                    ...
                    if(!response.ok) {
                        if(response.status==405) {
                            callback("Method not supported by remote endpoint.")
                            return;
                        }
                        callback("Could not communicate with remote endpoint.")
                        return;
                    }
                    return response.json();
                }).catch(error => {callback("Error running method remotely.")})
                .then(response => {if(response) callback(null, response)});
            }
        };
            ...
            if(walletType=="external") {
                ...
                if(window.web3.currentProvider) {
                    ...
                    if((window.web3.eth.accounts||[]).length==0) {
                        const err="External/Metamask provider is locked. Can't proceed."
                        alert(err);
                        callback(err,null);
                        return;
                    }
                    ...
                    this.that.props.functions.modal.show({cancel: ()=>{return false}, render: () => {return modal;}});
    ...
    _openWallet=(obj, accountName, cb)=>{
        ...
        const getKey=()=>{
            this.that.props.functions.wallet.getKey(walletName, accountIndex, (status, key)=>{
                if(status==0) {
                    const address=this.that.props.functions.wallet.getAddress(walletName, accountIndex);
                    obj.account={
                        address: address,
                        key: key
                    };
        ...
        if(this.that.props.functions.wallet.isOpen(walletName)) {
            ...
            this.that.props.functions.wallet.openWallet(walletName, null, (status)=>{
                if(status==0) {
                    getKey();
                }
                else if(status==2) {
