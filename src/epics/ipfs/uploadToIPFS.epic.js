...
/**
 * As we don't want the user messing around with the autogenerated file, let's make it readonly
 * @param {ProjectItem} project The current active project
 */
const makeFileReadOnly = (project) => {
    const path = '/.super/ipfs.json'.split('/')
    project.getItemByPath(path, project)
        .then(item => item.setReadOnly(true));
}
        ...
        .pipe(
            ...
            switchMap(({shareURL, timestamp}) => from(backend.loadFilePromise(projectId, '/.super/ipfs.json'))
                .pipe(
                    ...
                    tap(() => makeFileReadOnly(router.control.getActiveProject())),
                    catchError(error => {
                        console.log(error);
